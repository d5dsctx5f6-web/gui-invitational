...
**Two editorial items flagged for Chris, neither blocking:**
1. Whether he wants the individual title's own specific tiebreaker ladder (cumulative net →
   Sunday net → Sunday back-9 net → chip-off) spelled out in the app copy, now that the
   inaccurate "(points, head-to-head, holes won) applies to all three" claim has been removed.
2. Whether to bump the internal `GUI_INVITATIONAL_RULEBOOK.md` to v1.7 to reflect Brief 21's
   skins cross-round carryover policy reversal — that doc is currently stale relative to
   `PRODUCT_SPEC_ADDENDUM_A.md` §2 (the app itself already correctly follows the newer addendum).
