# PROJECT STATUS — The Hedges Invitational

**Last updated:** August 29, 2026 · **Status:** M3 is closed; Brief 9 (admin/UX hardening), Brief 10 (score routing fix), Brief 11 (RM visibility investigation, no code defect found), Brief 12 (PIN modal + duo A/B slot picker), Brief 13 (duo resubmission fix + admin duo view/set/reset), Brief 14 (the live leaderboard — a new core screen), Brief 15 (stale leaderboard data fix + rounds admin cleanup), Brief 16 (leaderboard score display — net-to-par + gross), Brief 17 (tee time slotting), Brief 18 (null handicap index fix — a real correctness bug), Brief 19 (gross-sorted individual race toggle), Brief 20 (admin Corrections reorganized into a round → match → hole drill-down), Brief 21 (skins cross-round carryover — a documented policy reversal — plus a fix to which number the leaderboard toggle actually makes prominent), Brief 22 (skins opt-in cutoff now reads the player's own match tee time, no early offset), Brief 23 (Scorecard redesigned Ryder Cup style — collapsible full breakdown, a compact always-on up/down indicator, and a prominent hole-winner banner), Brief 24 (Sunday pairings preview on the leaderboard, a much more prominent Money round selector, and skins-vs-Challenge-Ledger color-coding in the running ledger), Brief 25 (the in-app Rulebook screen — a new player-facing `/rulebook` with 9 collapsible sections, plus one real accuracy fix found and corrected in the "how ties get broken" copy), Brief 26 (every tee time now explicitly anchored to Arizona time regardless of the viewing device's or server's own timezone, plus a Scorecard text sizing pass), Brief 27 (a real shared text-size scale — three CSS tokens in `globals.css` — migrated across every screen in the app, superseding Brief 26's scorecard-only scope), Brief 28 (Rulebook content expansion — more descriptive copy with worked examples, and both tiebreaker ladders finally spelled out correctly in full), Brief 29 (the mercy rule — a quadruple-bogey gross cap a scorekeeper can call on any hole, zero engine changes needed), Brief 30 (design system foundation — event renamed to **The Hedges Invitational**; a new `/design-preview` route proving the full light/dark token system, North/South Hedges colors, Inter + Barlow Condensed typography, and every Part E primitive component; the delivered logo mark wired into favicons/PWA icons; zero existing live screens touched beyond the rename text), and Brief 31 (schema + engine migration, v1.0 → v2.0 — the largest brief yet: `duo_submissions`/`skins_entries`/the old `matches` table dropped outright, `hole_scores`/`reverse_mulligans` rebuilt duo-scoped, a new `duos` table replacing both matches and duo submissions, two-team North/South standings, mercy cap and reverse-mulligan simplified to one-score-in-one-score-out, Pairings Night's declare/counter order derived from a single stored coin-flip fact — 60/60 rebuilt engine tests, backend complete but **not deployed**, see below) all built on top of it. **v2.0 note**: `PRODUCT_SPEC_V2.md`/`GUI_INVITATIONAL_RULEBOOK_V2.md` are canonical; the *live, deployed* app is still v1.0's four-team shamble/four-ball format, since Brief 31's schema/engine work is complete locally but held back from `origin/main` — pushing it would fail Vercel's build (4 live screens — Scorecard, Score routing, Money, Leaderboard — still import the now-deleted v1.0 engine functions; deliberately untouched, since rebuilding them against Brief 30's design system is Briefs 32-35's job, not Brief 31's). A failed Vercel build does not take down the currently-live production site — it just won't promote the new version. **Pushed to `origin/main` at Chris's explicit direction** (a red build until Brief 32+ rebuilds those 4 screens is expected, not a defect); Chris runs the actual migration in the Supabase SQL editor next. The short-handed-team question (7 vs. 8 players) is **not** an open item — it was already settled before Brief 31 started (commissioner decides live, no fixed formula); the schema Brief 31 built (no forced-8 CHECK, nullable `duos.player_2_id`) is exactly the right shape for that and needs nothing further — corrected here after an earlier report of mine wrongly called it still-open. Outstanding, pre-v2.0: Brief 7's live two-device gate (now also covers `/leaderboard`'s realtime), Brief 9's own live verification, Chris's own live click-through of the duo slot picker as a real captain, confirming the resubmission-before-reveal assumption from Brief 13, Chris confirming Brief 20's actual write path on his own device, Chris confirming on `/money` that a real unresolved Saturday skins tail actually carries into Sunday's pool once the real trip produces one, Chris confirming two players in different-tee-time matches see correctly different skins cutoffs once a round has more than one matchup posted, Chris's own live click-through of the redesigned Scorecard as a real player (now including the mercy chip), Chris glancing at the real Sunday pairings preview once production has all 4 teams and a shamble round, Chris confirming Brief 30's masthead-lockup interpretation (favicon/manifest done; the actual home/champions masthead text was renamed but not given the logo image — see `SESSION_ADDENDUM_BRIEF30.md`), and **running migrations `0020`/`0021`/`0022`/`0023`/`0024`** — all independent and can happen on their own schedule — read this first before resuming.
...
- **Brief 27 (app-wide text sizing — a real shared scale) — CLOSED, supersedes Brief 26's scorecard-only scope**: three CSS custom properties added to `app/globals.css` — `--fs-label` (12px, secondary/meta text), `--fs-body` (14px, primary readable content), `--fs-emphasis` (18px, prominent short numbers) — documented in place so a future new screen naturally reaches for them instead of needing its own sizing-pass brief. Checked the original mockup's own type scale first and deliberately didn't inherit it (it skews toward 8-11px, closer to *why* Chris's complaint exists than a legibility floor); used Brief 26's already-verified corrections as the real floor instead. Migrated the scorecard onto the new tokens (Part B, confirming no regression from Brief 26), then rolled the scale out across `/duos`, `/money`, `/leaderboard`, `/rulebook`, `/schedule` (Part C) — plus `/champions` and the shared `SignInModal`, both a small, disclosed scope extension since they're the same category of player-facing UI even though not named in the brief's own list. Admin (lower priority per the brief) got the same treatment where layout allows — `.input`/`.select` were already exactly 12px so migrating them was a zero-risk token-ify, while `.btn`/`.btnGhost`/`.btnDanger` went from 11px to the label tier, the one real increase inside admin's densest packed-inline-form rows. 97 tests, unchanged (no engine touched). Live-verified on a real 375×812 viewport across `/leaderboard`, `/rulebook`, `/schedule`, and — the real test — `/admin` signed in with the commissioner passcode, specifically checking the two rows the brief called out as highest-risk (Rounds & Matchups' packed matchup row, Corrections' packed per-player row): both rendered cleanly with room to spare, no wrapping or overflow. Also incidentally reconfirmed Brief 26's timezone fix is still holding (a real tee time round-tripped through the admin edit form as the correct Arizona time). `/duos`/`/money` migrated by the same already-proven-safe mechanical process but not re-verified live (requires signing in as a real player). See `SESSION_ADDENDUM_BRIEF27.md`.
- **Brief 28 (Rulebook content expansion) — CLOSED**: replaced all nine `/rulebook` section bodies with more descriptive, example-driven copy — same headers, same collapsible structure Brief 25 built and Brief 27 restyled. The one genuinely new piece of information: both tiebreaker ladders finally spelled out in full (Cup/Sunday-pairings use points → head-to-head → holes won; the individual title uses cumulative net → Sunday net → Sunday back-9 net) — Brief 25's original one-sentence attempt at this had been wrong and had to be stripped rather than shipped false. Cross-checked against `PRODUCT_SPEC_ADDENDUM_A.md` §3 once more before shipping and confirmed accurate this time, verbatim from the brief. 97 tests, unchanged (pure content change). Live-verified on a real mobile viewport — the longest expanded sections (Cup, Money) render cleanly on Brief 27's shared type scale with no overflow. See `SESSION_ADDENDUM_BRIEF28.md`.
- **Brief 29 (the mercy rule — quadruple bogey cap) — CLOSED**: a new `mercy_called` boolean on `hole_scores` (migration `0024`, same pattern as `breakfast_ball`/`mulligan`) lets a scorekeeper cap a player's gross score at that hole's par + 4 when he's not going to finish it. Part B traced the actual engine read paths before assuming and confirmed genuinely **zero engine changes needed** — `matchScore()`/`realScore()` both already read whatever `strokes` is on record, so capping it at entry flows correctly into match state (via best-ball), skins, and the individual race automatically. A new Mercy chip on the scorecard reuses Brief 27's exact chip styling (no new CSS) — tapping it auto-sets the stroke entry to par+4 and flips the chip's own label to "Mercy called," with no per-round limit unlike breakfast ball/mulligan. Admin Corrections gained a matching checkbox. `/rulebook` gained a new "Calling mercy" section, correctly inserted right after "Do-overs." All three of the brief's stated assumptions (opt-in not automatic, one flag not two, no stroke-count floor) were used as given, no overrides needed. 97 tests, unchanged — direct evidence Part B's diagnosis was right. Live-verified two ways: admin Corrections against real production data, which hasn't run migration `0024` yet — confirmed the whole page still loads correctly with every Mercy checkbox gracefully defaulting to unchecked (proving the decoupled-fetch pattern, not just reasoning about it); and a temporary fake-data QA route showing a real duo correctly staying **▲1 thru 1** after one partner's score was mercy-capped, because best-ball used the other partner's real score — direct proof of the downstream correctness Part B predicted. See `SESSION_ADDENDUM_BRIEF29.md`.
- **Brief 30 (design system foundation — light + dark, North/South Hedges) — CLOSED, status: infrastructure only, no live screen restyled**: the event renamed to **The Hedges Invitational** via a new `lib/config.ts` (`EVENT_NAME`, `TEAM_NAMES` — the latter not consumed yet, staged for Brief 31). Diagnosed the codebase's actual styling approach before writing anything, per the brief's own instruction: **CSS Modules + a global `globals.css`, not Tailwind** — no Tailwind dependency exists anywhere in `package.json`. Ported the architect-delivered `hedges-tokens.css` (locked color/type tokens — North Hedges `#00205B`/`#2C5EB0` dark, South Hedges `#B9975B` both themes, Inter + Barlow Condensed, a real independently-authored `[data-theme="dark"]` block) into a new isolated `/design-preview` route (dev-only, not linked from nav) via a **nested** layout, so Next's per-route CSS/font bundling keeps it from ever reaching any existing screen. Added a spacing/radius/motion scale of its own (Part D wasn't locked in the brief) derived from the actual pixel values used throughout the approved mockup. Built all eight Part E primitives (Button, Card, Chip, Stat tile, Match state pill, Stepper, Tab bar, Theme toggle) as CSS Modules components matching the existing codebase convention. The theme toggle uses `useSyncExternalStore` rather than `useState`+`useEffect` — this repo's ESLint config hard-errors on `react-hooks/set-state-in-effect`, and `useSyncExternalStore` is the correct React primitive for reading `localStorage`/`matchMedia` safely across server/client anyway, not a workaround. Wired the delivered logo mark into favicons, PWA manifest icons, and apple-touch-icon (new reusable `scripts/generate-icons.ts`, using `sharp` — added as a new devDependency); **found and fixed a real defect in the delivered `mark-color-dark-bg.svg`** — its own comment contained an illegal `--` sequence for XML comments, which silently broke the file as an `<img>` source in strict browsers despite fetching fine as raw text; fixed in the working copy, left as-delivered in the pristine `design/logo/` reference copy, flagged for Chris to fix at the source. **One disclosed interpretation call**: wired the mark into favicon/manifest (meta-level, doesn't change how any screen looks) but did **not** insert the logo image into the actual home/`/champions` masthead JSX, since the brief's own gate says this brief touches zero existing live screens beyond what Part A's rename explicitly requires — the masthead-with-logo lockup is instead demonstrated live inside `/design-preview` itself. 97 tests, unchanged (zero engine involvement). Live-verified: `/design-preview` in both light and dark mode (screenshotted), North/South Hedges colors checked for real contrast against both neutral scales, toggle preference confirmed surviving both a full reload and a cold second browser tab, and — critically — `/` and `/rulebook` re-checked afterward and confirmed visually/functionally unchanged (network inspection confirmed neither route ever loads `/design-preview`'s CSS or fonts). **`/design-preview`'s phone verification was explicitly waived**: dev-only internal route, never player-facing, so browser-session verification (both themes, contrast checked, toggle survives reload + a cold tab) was accepted by Chris as sufficient — the phone-gate itself stays mandatory and unchanged for every player-facing screen from Brief 32 onward. See `SESSION_ADDENDUM_BRIEF30.md`.
- **PWA white-screen report — diagnosed and fixed**: Chris reported the installed PWA showing a white screen from the home-screen icon. Diagnosis found `manifest.json`, all three icon files, and `start_url`/`scope` all valid and correct on production; confirmed **no service worker has ever existed anywhere in this codebase's history** (grepped code + full git log — nothing to serve a stale cache); confirmed zero console error loading the deployed production URL fresh in a real browser tab. What the diagnosis *did* find, verified by reading the code rather than guessing: **this app has never had a Next.js error boundary anywhere** — meaning any unhandled client-side render error, from any cause, has always silently blanked the whole screen with nothing to recover with, by React/Next's own default. Leading hypothesis for the specific trigger (labeled explicitly as a hypothesis, not confirmed without Chris's own device console): a PWA session left open/suspended since before a deploy, resumed by the OS from memory, hitting a JS chunk from the now-replaced previous build. Fixed the structural gap either way: new `app/error.tsx` (+ `.module.css`, on-brand spruce/gold styling) and `app/global-error.tsx` (self-contained, inline-styled, since it replaces the root layout entirely per Next's own requirement) — both now show a "Something went wrong — Try again" screen instead of blanking. **Actually triggered the failure mode to prove the fix**, not just trusted the docs: a temporary QA route that unconditionally threw confirmed `app/error.tsx` catches it and renders correctly, then was deleted before commit. Re-ran Brief 30's own gate afterward — `/`, `/rulebook`, `/design-preview` all re-confirmed unchanged. 97 tests, unchanged. **Open**: asked Chris to force-quit-and-reopen (or remove/re-add) the installed icon as a cheap confirming test, and to get the actual on-device console error via remote debugging if it recurs. See `SESSION_ADDENDUM_PWA_WHITE_SCREEN_FIX.md`.
- **Brief 31 (schema + engine migration, v1.0 → v2.0 format) — backend CLOSED, pushed, migrations pending Chris's own run**: the largest brief in the project's history — "almost none of v1.0's schema/engine survives" per the brief's own framing, and it didn't. **Part A (schema, `0025_v2_schema_migration.sql`)**: dropped `duo_submissions`/`skins_entries`/the old `matches` table outright (no successor); dropped `rounds.format` and `rounds.skins_buy_in` (the latter not named in the brief, but orphaned by the same drop); cleared and re-constrained `teams`/`team_members` to exactly North Hedges/South Hedges (`check` + `unique`, not four admin-named teams); rebuilt `hole_scores` and `reverse_mulligans` duo-scoped (drop + recreate — no valid old-row mapping existed); new `duos` table (round-scoped, replaces `matches` + `duo_submissions` together); `seasons` gained the one raw coin-flip fact plus a chip-off result column. **Part B (engine)**: deleted `skins.ts`/`individualRace.ts`/`config.ts`/`netScore.ts` outright; rewrote `matchState.ts` (duo-scoped, mercy cap applied at comparison via new `mercyCap.ts`), `standings.ts` (two-team, head-to-head tiebreaker step removed as mathematically redundant with only two teams), `reverseMulligan.ts` (team→duo), `pairings.ts` (same filename, entirely repurposed — was earned-Sunday-pairings-from-standings, now Friday/Sunday declare-counter order derived from the stored coin flip), `handicap.ts` (kept `courseHandicap()` only, explicit DISPLAY-ONLY boundary comment; dropped the stroke-allocation pipeline that has no meaning left with no strokes given anywhere), `moneyLedger.ts` (dropped `skinsPayouts()`, simplified `runningLedger()` to bets-only). New `drivesUsed.ts`. **Part C (tests)**: 97→60 tests, 13→11 files, every required new category covered (full 18-hole scramble match, mercy cap proven both applied and non-cosmetic, RM one-score-in-one-score-out, a genuine 12-12-style chip-off tie, pairings order provably derived not independently computed, Drives Used tally, count-agnostic absence handling) — two real bugs caught in my own new test expectations (an early-close miscalculation, and an impossible "cap creates a win" premise), not in the engine. **Verification**: row counts confirmed and real data (fully archived, see Brief 31's own addendum) before any drop; RLS + realtime re-added on every new/recreated table; isolation rule confirmed clean (zero framework/Supabase imports anywhere in `/engine`); `courseHandicap()` confirmed to have zero call sites inside match-state computation — in fact zero call sites anywhere outside its own module and tests. `ARCHITECTURE.md` §5 reconciled; `supabase/README.md`'s table map rewritten for v2.0 with an explicit retired-tables section. **Build note, not a defect**: `npm run build` fails in exactly 4 untouched live screens (`Scorecard.tsx`, `score/page.tsx`, `MoneyScreen.tsx`, `LeaderboardScreen.tsx`) that still import now-deleted v1.0 engine functions — rebuilding them is Briefs 32-35's job, not this one's, per the brief's own "backend only, no screen changes" scope. **Pushed to `origin/main` at Chris's explicit direction** — a temporarily red Vercel build is expected and accepted until Brief 32+ rebuilds those screens; Vercel doesn't promote a failed build, so the live site is unaffected either way. Chris runs the actual migration (`0025`) himself in the Supabase SQL editor next, same as every migration before it. The brief's own open question (7-vs-8 players) was already settled going in — commissioner decides live, no fixed formula — the schema built here (no forced-8 CHECK, nullable `duos.player_2_id`) is exactly right for that, not actually still open despite an earlier report of mine calling it so. See `SESSION_ADDENDUM_BRIEF31.md`.
...
## Brief 27 (app-wide text sizing — a real shared scale) status: CLOSED

Built, verified (build/lint/97 tests unchanged — no engine touched — plus live verification on a
real mobile viewport across the public screens and a signed-in admin check of the two densest
layouts the brief flagged), and pushed. See `SESSION_ADDENDUM_BRIEF27.md`. Supersedes Brief 26's
scorecard-only text sizing scope — this is now the permanent, app-wide standard.

**Still pending:** a live signed-in check of `/duos` and `/money` on Chris's own device — this
session migrated them via the same mechanical process already proven safe on every other screen,
but didn't re-verify live since that requires a real player sign-in, per this project's standing
identity-squatting-avoidance policy. Also open, not blocking: whether Chris wants the home page
(`app/page.module.css`, not in this brief's named scope) brought onto the same scale for full
consistency — a small, low-risk follow-up now that the scale exists.

## Brief 28 (Rulebook content expansion) status: CLOSED

Built, verified (build/lint/97 tests unchanged — pure content, no engine touched — plus live
verification on a real mobile viewport), and pushed. See `SESSION_ADDENDUM_BRIEF28.md`. No open
items specific to this brief.

## Brief 29 (the mercy rule — quadruple bogey cap) status: CLOSED, ONE MIGRATION PENDING

Built, verified (build/lint/97 tests unchanged — confirming Part B's zero-engine-change diagnosis
— plus live verification against real production data and a temporary fake-data QA route), and
pushed. See `SESSION_ADDENDUM_BRIEF29.md`.

**Still pending:** migration `0024` needs Chris to run it — now five migrations queued (`0020`,
`0021`, `0022`, `0023`, `0024`), only `0020`/`0022`/`0023` confirmed run so far. The feature
already degrades gracefully without it (live-verified: admin Corrections loads fine against the
current, pre-migration production database, mercy just always reads as unclaimed until the column
exists) — not blocking, but the mercy flag can't actually be saved for real until it runs.
